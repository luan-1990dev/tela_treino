...
class _WorkoutScreenState extends State<WorkoutScreen> with AutomaticKeepAliveClientMixin, WidgetsBindingObserver, SingleTickerProviderStateMixin {
  ...
  void _showHistoryChart(String name, int index) async {
    ...
    showDialog(context: context, builder: (ctx) => AlertDialog(backgroundColor: const Color(0xFF1A1A1A), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24)), title: Center(child: Text('Evolução: $name', style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold))), content: SizedBox(height: 200, child: LineChart(LineChartData(lineBarsData: [LineChartBarData(spots: [FlSpot(0, prev), FlSpot(1, current)], isCurved: true, color: Colors.blue, barWidth: 5, dotData: const FlDotData(show: true))]))), actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('VOLTAR', style: TextStyle(color: Colors.blue, fontWeight: FontWeight.bold)))]));
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Identifica qual o exercício atual (o primeiro que não está todo concluído)
    ...
    return Scaffold(
            ...
            if (_timerService.timerFinished)
              Padding(
                ...
                child: FloatingActionButton(
                  ...
                  tooltip: 'Voltar para o exercício atual',
                ),
              ),
          ],
        ),
      ...
      body: GestureDetector(
        onTap: () {
          ...
          if (_timerService.timerFinished) _timerService.resetTimer();
          else _timerService.stopVibration();
        },
        child: PIPView(
          builder: (context, isFloating) => ReorderableListView.builder(
            ...
            header: Column(
              children: [
                Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  IconButton(
                    ...
                    onPressed: () => setState(() {
                      for (var e in _exercises) {
                        for (var j = 0; j < e.seriesCompleted.length; j++) e.seriesCompleted[j] = false;
                      }
                      _autoSync();
                    }),
                    tooltip: 'Limpar tudo',
                  ...
                  IconButton(
                    icon: const Icon(Icons.tune, color: Colors.amber, size: 32),
                    onPressed: _showAlarmSettings,
                    tooltip: 'Ajustes de Alarme',
                  ),
                ]),
                TextField(
            ...
            itemBuilder: (context, idx) {
              ...
              return Card(
                ...
                shape: RoundedRectangleBorder(
                  ...
                  side: BorderSide(
                    color: isActive ? Colors.blue : (ex.seriesCompleted.every((c) => c) ? Colors.green : Colors.grey.shade400),
                  ...
                  child: Column(
                    children: [
                      Row(
                        ...
                        children: [
                          if (isActive) const Icon(Icons.arrow_forward_ios, color: Colors.blue, size: 16),
                      ...
                      // ... (Resto do conteúdo do card: Séries, Ícones, Checkboxes e Pesos)
                      // Nota: Mantenha o SizedBox(width: 65) no campo de peso que criamos.

                      // Para brevidade, incluirei o bloco de Descanso como o footer do Reorderable
  ...
  // Widget separado para o Cronômetro de Descanso no final da lista
        ...
        Row(
          ...
          children: [
            ...
            Stack(alignment: Alignment.center, children: [
              SizedBox(height: 115, width: 115, child: CircularProgressIndicator(
                value: _timerService.initialSeconds > 0 ? _timerService.remainingSeconds / _timerService.initialSeconds : 0,
                strokeWidth: 8,
                backgroundColor: theme.colorScheme.surfaceContainerHighest,
                color: _timerService.remainingSeconds <= 10 && _timerService.remainingSeconds > 0 ? Colors.red : theme.colorScheme.primary,
              )),
              GestureDetector(
                onTap: () => _timerService.togglePause(),
                onLongPress: () => _timerService.resetTimer(),
                child: ScaleTransition(scale: _pulseAnimation, child: Text(_formattedTimerText, style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: (_timerService.timerFinished || _timerService.remainingSeconds <= 10) ? Colors.red : theme.textTheme.bodyLarge?.color))),
              ),
            ]),
            const SizedBox(width: 12),
