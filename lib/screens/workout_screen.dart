...
class _WorkoutScreenState extends State<WorkoutScreen> with AutomaticKeepAliveClientMixin, WidgetsBindingObserver, SingleTickerProviderStateMixin {
  ...
  void _showInactivitySnackBar() {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      duration: const Duration(seconds: 8), backgroundColor: Colors.blueGrey.shade900, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      content: const Text("Como você está se sentindo?\nUm pouco mais cansado (a)?\nNão desista! Todo esforço vai valer a pena.", textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold)),
    ));
  }
  ...
  Future<void> _saveState(int index) async {
    ...
    // SNACKBAR MOTIVACIONAL: Apenas no último card marcado inteiramente
  ...
  void _showMotivationalSnackBar() {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(duration: const Duration(seconds: 5), behavior: SnackBarBehavior.floating, backgroundColor: Colors.orange.shade800, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), content: const Text("Agora falta pouco, só falta mais um treino para a glória eterna! 🏆", style: TextStyle(fontWeight: FontWeight.bold))));
  }
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          Text(arrow, style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: statusColor)), const SizedBox(width: 8),
          Text('${currentVolume.toStringAsFixed(1)} kg', style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: statusColor)),
        ]),
        Text('${diff.abs().toStringAsFixed(1)} kg $trendText', style: TextStyle(fontSize: 14, color: statusColor.withOpacity(0.8))), const SizedBox(height: 20),
  ...
  void _scrollToPending() {
    ...
    // Procura o primeiro card que não está totalmente concluído
    ...
    // Se estivermos no último card e ele estiver incompleto, avisa. 
    // Se for o meio do treino, apenas rola para o último marcado silenciosamente.
    if (pendingIndex != -1 && pendingIndex == _exercises.length - 1) {
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Atenção: "$pendingName" possui séries pendentes! 🏋️‍♂️'),
  ...
  void _requestRemove(int index) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        Text('Excluir "$name"?', style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold)), const SizedBox(height: 12),
          ...
          child: Column(children: [
            ...
            ...List.generate(_exercises.length, (idx) {
              ...
              return Card(
                ...
                child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    IconButton(icon: const Icon(Icons.remove_circle_outline, size: 24, color: Colors.grey), onPressed: () { setState(() => ex.updateSeriesCount(ex.seriesCompleted.length - 1)); _saveState(idx); }, tooltip: 'Diminuir séries'),
                    ...
                    IconButton(icon: const Icon(Icons.add_circle_outline, size: 24, color: Colors.blue), onPressed: () { setState(() => ex.updateSeriesCount(ex.seriesCompleted.length + 1)); _saveState(idx); }, tooltip: 'Aumentar séries'),
                  ]),
                  Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    IconButton(icon: const Icon(Icons.ondemand_video, color: Colors.blueGrey, size: 24), onPressed: () => launchUrl(Uri.parse('https://www.youtube.com/results?search_query=${ex.nameController.text}'), mode: LaunchMode.externalApplication), tooltip: 'Tutorial YouTube'),
                  ...
                  ...List.generate(ex.repsControllers.length, (idx2) => Padding(
                    padding: const EdgeInsets.symmetric(vertical: 4),
                    child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      CircleAvatar(radius: 10, child: Text('${idx2 + 1}', style: const TextStyle(fontSize: 10))),
                      const SizedBox(width: 12),
                      const Text('Rep:'),
                      SizedBox(width: 35, child: TextField(controller: ex.repsControllers[idx2], textAlign: TextAlign.center, decoration: const InputDecoration(isDense: true), keyboardType: TextInputType.number, onChanged: (v) => _saveState(idx))),
                      const SizedBox(width: 16),
                      const Text('Peso:'),
                      SizedBox(width: 55, child: TextField(controller: ex.weightControllers[idx2], textAlign: TextAlign.center, decoration: const InputDecoration(isDense: true, suffixText: 'kg'), keyboardType: TextInputType.number, onChanged: (v) { _db.insertHistory(ex.nameController.text, double.tryParse(v) ?? 0); _saveState(idx); })),
                    ]),
                  )),
                ])),
              );
            ...
            Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Stack(alignment: Alignment.center, children: [
                ...
                GestureDetector(onTap: () => _timerService.togglePause(), onLongPress: () => _timerService.resetTimer(), child: ScaleTransition(scale: _pulseAnimation, child: Text('${(_timerService.remainingSeconds ~/ 60).toString().padLeft(2, '0')}:${(_timerService.remainingSeconds % 60).toString().padLeft(2, '0')}', style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: _timerService.timerFinished ? (theme.brightness == Brightness.dark ? Colors.white : Colors.black) : ((_timerService.remainingSeconds > 0 && _timerService.remainingSeconds <= 10) ? Colors.red : theme.textTheme.bodyLarge?.color))))),
              ]),
